...
MHI_species_features = read.csv("MHI_features_species.txt")
MHI_S_F = data.frame(MHI_species_features)
head(MHI_S_F)

# PCA for species richness  (or species presence or species abundance - which is most appropriate?) vs features
MHI_S_F_pca = prcomp(MHI_S_F, scale=TRUE)
summary(MHI_S_F_pca)
